...
function WatchlistScreen({ movies, genres, isDarkMode, onAddToSeen, onRemoveFromWatchlist }) {
  ...
  /**
   * Fetch streaming providers on component mount and when media type changes
   */
  useEffect(() => {
  ...
  const filteredMovies = useMemo(() => {
    ...
    if (selectedDecades.length > 0) {
      filtered = filtered.filter(movie => {
        // Handle both release_date (movies) and first_air_date (TV shows)
    ...
    return (
      <SafeAreaView style={[layoutStyles.safeArea, { backgroundColor: isDarkMode ? '#1C2526' : '#FFFFFF' }]}>
        <View style={stateStyles.emptyStateContainer}>
          <Ionicons 
            name={mediaType === 'movie' ? "eye-off-outline" : "tv-outline"} 
            size={64} 
    ...
    <SafeAreaView style={[layoutStyles.safeArea, { backgroundColor: isDarkMode ? '#1C2526' : '#FFFFFF' }]}>
      ...
      <View style={[headerStyles.screenHeader, { backgroundColor: isDarkMode ? '#4B0082' : '#F5F5F5', borderBottomColor: isDarkMode ? '#8A2BE2' : '#E0E0E0' }]}>
        <Text style={[headerStyles.screenTitle, { color: isDarkMode ? '#F5F5F5' : '#333' }]}>
          Top Rated {mediaType === 'movie' ? 'Movies' : 'TV Shows'} You Haven't Seen
        </Text>
            ...
            <View style={styles.cardContent}>
              ...
              <View style={styles.movieDetails}>
                ...
                <Text style={[styles.movieScore, { color: isDarkMode ? '#FFD700' : '#4B0082' }]}>
                  {movie.score ? movie.score.toFixed(1) : movie.vote_average ? movie.vote_average.toFixed(1) : '0.0'}
                </Text>
