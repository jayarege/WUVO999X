...
function WildcardScreen({
 ...
 const buildFilteredMoviePool = useCallback(async () => {
  ...
  /**
   * Get a movie from the filtered pool
   */
  const getMovieFromFilteredPool = useCallback(async () => {
    if (filteredMoviePool.length === 0) {
      throw new Error('No movies found matching your filters. Try adjusting your filter settings or building the movie pool first.');
    }

    // Remove movies we've already used
    const excludedIds = new Set();
    seen.forEach(movie => excludedIds.add(movie.id));
    unseen.forEach(movie => excludedIds.add(movie.id));
    comparedMovies.forEach(id => excludedIds.add(id));

    const availableMovies = filteredMoviePool.filter(movie => !excludedIds.has(movie.id));
    
    if (availableMovies.length === 0) {
      throw new Error('All movies in the current filter have been used. Try adjusting your filters or adding more movies to your ratings.');
    }

    const randomMovie = availableMovies[Math.floor(Math.random() * availableMovies.length)];
    console.log(`🎲 Selected from pool: ${randomMovie.title}`);

    // If the movie doesn't have streaming data, fetch it
    if (!randomMovie.streamingServices) {
      const streamingServices = await getMovieStreamingData(randomMovie.id);
      return {
        id: randomMovie.id,
    ...
    return {
      ...
      eloRating: randomMovie.vote_average * 10,
      userRating: randomMovie.vote_average,
      streamingServices: randomMovie.streamingServices
    };
  }, [filteredMoviePool, seen, unseen, comparedMovies, getMovieStreamingData]);

  /**
   * Get movie details with streaming data
   */
  const getMovieDetails = useCallback(async (movieId) => {
    try {
      ...
      const data = await response.json();
      const streamingServices = await getMovieStreamingData(movieId);
  ...
  const getFilteredMovie = useCallback(async () => {
    ...
    if (eligibleMovies.length === 0) {
      throw new Error('No movies found matching your filters');
    }
    const randomMovie = eligibleMovies[Math.floor(Math.random() * eligibleMovies.length)];
    return await getMovieDetails(randomMovie.id);
  }, [selectedDecades, selectedGenres, seen, unseen, comparedMovies, skippedMovies, getMovieDetails]);

  /**
  // ... (Include all the other functions: handleReset, fetchStreamingProviders, updateMovieRating, getNextBaselineMovieId, markMovieAsCompared, calculateKFactor, adjustRating, etc. - keeping them exactly the same)
...
const newMovieData = await getFilteredMovie();
        ...
        if (!baselineComplete) {
          ...
          console.log('Baseline complete - Using popular movies');
      ...
      if (!newMovieData || newMovieData.id === randomSeenMovie.id) {
        throw new Error('Failed to find a suitable movie for comparison');
      }
      
      console.log('✅ Selected new movie:', newMovieData.title);
      safeSetState(setNewMovie, newMovieData);
      resetLoadingState();
    ...
    getMovieFromFilteredPool, filteredMoviePool, isLoadingFilteredPool,
    ...
    return (
      <SafeAreaView style={[layoutStyles.safeArea, { backgroundColor: isDarkMode ? '#1C2526' : '#FFFFFF' }]}>
        <View style={stateStyles.loadingContainer}>
          ...
          {hasActiveFilters && (
            <Text style={[stateStyles.loadingText, { color: isDarkMode ? '#D3D3D3' : '#666', fontSize: 12, marginTop: 5 }]}>
              Active filters: {[
          ...
          <View style={compareStyles.compareMovies}>
            ...
            >
              ...
              <View style={compareStyles.posterOverlay}>
                ...
                <View style={styles.overlayStreamingContainer}>
                  {seenMovie.streamingServices && seenMovie.streamingServices.slice(0, 3).map((service, index) => {
                    ...
                    if (serviceData && serviceData.logo_url) {
                      return (
                        <Image
                          ...
                          style={[
                            styles.overlayStreamingIcon,
                            service.paymentType === 'paid' && styles.paidStreamingIcon,
                            service.paymentType === 'free' && styles.freeStreamingIcon
                          ]}
                          resizeMode="contain"
                        />
                      );
            ...
            >
              ...
              <View style={compareStyles.posterOverlay}>
                ...
                {/* Streaming service icons */}
                <View style={styles.overlayStreamingContainer}>
                  {newMovie.streamingServices && newMovie.streamingServices.slice(0, 3).map((service, index) => {
                    const serviceData = streamingProviders.find(s => s.id === service.provider_id);
                    if (serviceData && serviceData.logo_url) {
                      return (
                        <Image
...
// Component styles with proper modal overlay
const styles = StyleSheet.create({
  ...
  overlayStreamingContainer: {
    flexDirection: 'row',
    alignItems: 'center',
    marginVertical: 4,
  },
  overlayStreamingIcon: {
    width: 20,
    height: 20,
    borderRadius: 3,
    marginRight: 5,
