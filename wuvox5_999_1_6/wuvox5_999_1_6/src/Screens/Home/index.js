...
function HomeScreen({ seen, unseen, genres, newReleases, isDarkMode, toggleTheme, onAddToSeen, onAddToUnseen, onRemoveFromWatchlist }) {
...
const recommendations = await getAIRecommendations(
  ...
  const fetchPopularMovies = useCallback(async () => {
    try {
      ...
      // Get top 10 most popular content (already sorted by TMDB popularity)
    ...
    <View style={[homeStyles.movieCardBorder, { width: 320, alignSelf: 'center', height: 150 }]}>
      ...
      >
        ...
        <View style={[homeStyles.movieInfoBox, { flex: 1, padding: 12, minWidth: 200, alignItems: 'center', justifyContent: 'center', height: 150 }]}>
          <Text
            style={[homeStyles.genreName, { fontSize: 13, lineHeight: 15, textAlign: 'center' }]}
            numberOfLines={1}
            ellipsizeMode="tail"
          >
            {item.title}
 ...
 const renderAIRecommendationsSection = useCallback(() => {
        ...
        {isLoadingRecommendations ? (
          ...
          <Animated.FlatList
            ...
            renderItem={({ item, index }) => (
              <View style={[homeStyles.movieCardBorder, { padding: 2 }]}>
                <TouchableOpacity
                ...
                >
                  <View
                    style={[homeStyles.enhancedCard, { width: MOVIE_CARD_WIDTH, height: MOVIE_CARD_WIDTH * 1.9, padding: 3 }]}
                  >
                    <View style={[styles.aiRecommendationBadge, { backgroundColor: '#4CAF50' }]}>
                    ...
                    <Image
                      ...
                      resizeMode="cover"
                    />
                    <View style={[homeStyles.movieInfoBox, { height: 60, width: MOVIE_CARD_WIDTH - 6, padding: 4 }]}>
                      <Text
                        style={[homeStyles.genreName, { fontSize: 12, lineHeight: 14, marginBottom: 2 }]}
                        numberOfLines={2}
                        ellipsizeMode="tail"
                      >
                        {item.title}
                      </Text>
                      <View style={[styles.ratingContainer, { marginTop: 2 }]}>
                        <Ionicons
                          name="star"
                          size={12}
  ...
  return (
    ...
    {/* Content Type Toggle (Movies/TV Shows) - FIXED VERSION */}
<View style={{
  marginTop: 12,
  marginBottom: 8,
  ...
  <View style={{
    borderWidth: 2,
    borderColor: mediaType === 'movie' ? '#6C2BD9' : '#59ADE8',
    borderRadius: 25,
    padding: 2,
    ...
    <View style={{
      ...
      <TouchableOpacity 
        style={{
          ...
          backgroundColor: contentType === 'movies' ? '#FFFFFF' : 'transparent',
        }}
      ...
      <TouchableOpacity 
        style={{
          ...
          backgroundColor: contentType === 'tv' ? '#FFFFFF' : 'transparent',
        }}
            ...
            <ScrollView style={homeStyles.homeContainer}>
              ...
              <View style={homeStyles.section}>
                ...
                {isLoadingRecent ? (
                  <View style={styles.loadingContainer}>
                    <Text style={homeStyles.swipeInstructions}>
                      Loading recent TV show releases...
                    </Text>
                  </View>
                ) : recentReleases.length === 0 ? (
                  <Text style={homeStyles.swipeInstructions}>
                    No new TV show releases found this week
                  </Text>
                ) : (
                  <FlatList
                    data={recentReleases}
