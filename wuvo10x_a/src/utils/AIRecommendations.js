...
class AIRecommendationService {
  ...
  async getGroqRecommendations(userMovies, mediaType = 'movie') {
    try {
      ...
      // Get negative feedback to improve recommendations
      ...
      try {
        ...
        if (stored) {
          negativeFeedback = JSON.parse(stored);
        }
      } catch (e) {
        console.log('No negative feedback found');
      }
      ...
      // Create negative feedback context for the AI
      let negativeContext = '';
      if (negativeFeedback.length > 0) {
        const recentNegative = negativeFeedback
          .slice(-20) // Get last 20 negative items
          .map(item => item.title)
          .join(', ');
        negativeContext = `\n\nIMPORTANT: The user was NOT interested in these ${contentType}: ${recentNegative}. Avoid recommending similar ${contentType} or ${contentType} with similar themes/genres.`;
      }

      const prompt = `Based on these highly rated ${contentType}, recommend 10 similar ${contentType} this user would love:

${movieList}${negativeContext}

Return only ${contentType} titles, one per line, no explanations or numbering. Focus on ${contentType} with similar themes, directors, or genres to the highly rated ones, but avoid anything similar to the user's dislikes.`;
...
// Helper method to filter out movies user has already seen or marked as not interested
...
export const getAIRecommendations = async (userMovies, mediaType, seenList, unseenList, skippedList = []) => {
  try {
    ...
    return aiRecommendationService.filterUnseenContent(recommendations, seenList, unseenList, skippedList);
  } catch (error) {
