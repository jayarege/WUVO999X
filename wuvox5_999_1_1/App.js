...
import { isDevModeEnabled, getDevMovies, getDevUser } from './src/utils/DevConfig';
...
export default function App() {
  ...
  const loadUserData = useCallback(async () => {
    try {
      // DEV MODE: Use pre-configured movies
      if (isDevModeEnabled()) {
        console.log('🔧 DEV MODE: Loading pre-configured movies');
  ...
  // CENTRAL LOCATION A - Update movie rating by ID
  const handleUpdateRating = useCallback((movieId, newRating) => {
    console.log(`🏪 CENTRAL STORE: Updating movie ${movieId} to rating ${newRating}`);
    setSeen(prev => 
      prev.map(movie => 
        movie.id === movieId 
  ...
  // CENTRAL LOCATION A - Add new movie to seen list (block adult content)
  const handleAddToSeen = useCallback((movie) => {
    if (movie.adult === true) {
      console.log(`🚫 REJECTED: ${movie.title} - Adult content`);
      Alert.alert('Content Filtered', 'Adult content is not allowed.');
      return;
    }
    
    console.log(`🏪 CENTRAL STORE: Adding/updating movie ${movie.title}`);
  ...
  const handleAddToUnseen = useCallback((movie) => {
    ...
    console.log(`🏪 CENTRAL STORE: Adding movie ${movie.title} to watchlist`);
  ...
  const handleRemoveFromWatchlist = useCallback((movieId) => {
    console.log(`🏪 CENTRAL STORE: Removing movie ${movieId} from watchlist`);
    setUnseen(prev => prev.filter(movie => movie.id !== movieId));
  }, []);
